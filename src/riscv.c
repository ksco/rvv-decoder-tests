...
int main() {
    ...
    code = malloc(sizeof(uint8_t) * (bufsize + 1));
    fseek(fp, 0L, SEEK_SET);
    long size = fread(code, sizeof(uint8_t), bufsize, fp);
    fclose(fp);
    disassemble_info info = {
        .print_insn = print_insn_riscv64,
        .read_memory_func = host_read_memory,
        .memory_error_func = perror_memory,
        .print_address_func = host_print_address,
        .symbol_at_address_func = symbol_at_address,
        .endian = BFD_ENDIAN_LITTLE,
        .fprintf_func = fprintf,
        .stream = stdout,
        .buffer = (void *)code,
        .buffer_vma = (uintptr_t)code,
        .buffer_length = size
    };
